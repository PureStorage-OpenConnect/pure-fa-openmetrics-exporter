import pytest
from purity_fa_prometheus_exporter.flasharray_collector.flasharray_metrics import pod_performance_metrics

def test_pod_perf_name(mock_fa_client):
    pod_perf = pod_performance_metrics.PodPerformanceMetrics(mock_fa_client)
...
def test_pod_perf_val(mock_fa_client):
    pod_perf = pod_performance_metrics.PodPerformanceMetrics(mock_fa_client)
    for m in pod_perf.get_metrics():
        for s in m.samples:
