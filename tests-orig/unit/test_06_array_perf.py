import pytest
from purity_fa_prometheus_exporter.flasharray_collector.flasharray_metrics import array_performance_metrics

def test_array_performance_name(mock_fa_client):
    array_perf = array_performance_metrics.ArrayPerformanceMetrics(mock_fa_client)
    for m in array_perf.get_metrics():
        for s in m.samples:
            assert s.name in ['purefa_array_performance_latency_usec',
                              ...
                              'purefa_array_performance_qdepth']

def test_array_performance_labels(mock_fa_client):
    array_perf = array_performance_metrics.ArrayPerformanceMetrics(mock_fa_client)
    for m in array_perf.get_metrics():
        for s in m.samples:
            if s.name == 'purefa_array_performance_qdepth':
                assert s.labels == {}
...
def test_array_performance_val(mock_fa_client):
    array_perf = array_performance_metrics.ArrayPerformanceMetrics(mock_fa_client)
    for m in array_perf.get_metrics():
        for s in m.samples:
