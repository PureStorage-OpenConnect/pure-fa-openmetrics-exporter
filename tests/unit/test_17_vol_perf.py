from pure_fa_openmetrics_exporter.flasharray_collector.flasharray_metrics import volume_performance_metrics

def test_vol_perf_name(fa_client):
    vol_perf = volume_performance_metrics.VolumePerformanceMetrics(fa_client)
...
def test_vol_perf_val(fa_client):
    vol_perf = volume_performance_metrics.VolumePerformanceMetrics(fa_client)
    for m in vol_perf.get_metrics():
        for s in m.samples:
