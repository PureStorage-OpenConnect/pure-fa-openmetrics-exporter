...
def test_directory_perf_name(fa_client):
    dir_perf = directory_performance_metrics.DirectoryPerformanceMetrics(fa_client)
...
def test_directory_perf_labels(fa_client):
    dir_perf = directory_performance_metrics.DirectoryPerformanceMetrics(fa_client)
    for m in dir_perf.get_metrics():
        for s in m.samples:
...
def test_directory_perf_val(fa_client):
    dir_perf = directory_performance_metrics.DirectoryPerformanceMetrics(fa_client)
    for m in dir_perf.get_metrics():
        for s in m.samples:
