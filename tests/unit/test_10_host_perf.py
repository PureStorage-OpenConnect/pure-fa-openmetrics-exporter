from pure_fa_openmetrics_exporter.flasharray_collector.flasharray_metrics import host_performance_metrics

def test_host_perf_name(fa_client):
    host_perf = host_performance_metrics.HostPerformanceMetrics(fa_client)
...
def test_host_perf_val(fa_client):
    host_perf = host_performance_metrics.HostPerformanceMetrics(fa_client)
    for m in host_perf.get_metrics():
        for s in m.samples:
